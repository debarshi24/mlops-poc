...
Environment variables required (set in the CodeBuild Deploy project):
- S3_BUCKET              REQUIRED (artifact bucket where train uploads models/)
...
- S3_PREFIX_FOR_DEPLOYMENT optional, default "models/deployed"
"""
import os
import logging
import boto3
...
def ensure_model_tar(s3_client, model_s3_uri, bucket, s3_prefix_for_deployment="models/deployed"):
    """
...
def create_and_deploy(endpoint_name, model_data_s3_uri, role_arn, region, instance_type="ml.t3.medium"):
    ...
    # Recommend using source_dir + entry_point (entry_point should be a filename inside source_dir)
    source_dir = "src/model"          # must exist in the input artifact
    entry_point = "inference.py"      # this file should be inside source_dir
    ...
    model = SKLearnModel(
        ...
        framework_version=None,  # leave None unless you need a specific SKLearn container version
...
def main():
    ...
    s3_prefix_for_deployment = os.environ.get("S3_PREFIX_FOR_DEPLOYMENT", "models/deployed")
    ...
    model_data_s3_uri = ensure_model_tar(s3_client, explicit_model_s3, bucket, s3_prefix_for_deployment)
